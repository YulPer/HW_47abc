...
class SignUp {
	...
	static void validate(ExtentTest logger) throws Exception {
		...
		Common.writeInfoLine("First Name", p.getProperty("fname_value"), Common.getValue(By.id(p.getProperty("el_07"))),
				logger);

		// 02 :: Last Name
		Common.setValue(By.id(p.getProperty("el_09")), p.getProperty("lname_value"));
		Common.writeInfoLine("Last Name", p.getProperty("lname_value"), Common.getValue(By.id(p.getProperty("el_09"))),
				logger);

		// 03 :: Email Name
		Common.setValue(By.id(p.getProperty("el_11")), p.getProperty("email_value"));
		Common.writeInfoLine("Email", p.getProperty("email_value"), Common.getValue(By.id(p.getProperty("el_11"))),
				logger);

		// 04 :: Phone Name
		Common.setValue(By.id(p.getProperty("el_13")), p.getProperty("phone_value"));
		Common.writeInfoLine("Phone", p.getProperty("phone_value"), Common.getValue(By.id(p.getProperty("el_13"))),
				logger);
	}
}
